...
author: "Joshua Preston, William Mueller"
...
```{r setup, warning=FALSE, message=FALSE, echo=FALSE}
  ...
  # Setting global class and bin colors for plots
    ...
    class_colors_final <- c(
      "Early-Peak-BW" = "#000", # black
      "Stable BW"     = "#d53b59", # red
      "Mid-Peak-BW"   = "#53ca3e" # green
    )
    bin_colors <- c(
      "0" = "#74ae66", # green
      "1" = "#f2d14c", # yellow
...
# Load in input data
```{r read in censuses}
  # Paths are relative to this .rmd's own dir (95_healthcard_cod/R/), which is
  # rmarkdown's default knit dir. `..` = 95_healthcard_cod, `../..` = repo root.
  #
  # INPUTS:
  #   - healthcard xlsx : the ONLY input unique to this analysis. Not in the repo --
  #                       drop it in 95_healthcard_cod/data/ before running.
  #   - roster census   : the pipeline's own SLAM raw census (tag <-> idno)
  #   - class_census    : NOT frozen -- reconstructed here as the 04 census
  #                       (classes / le_wk) joined with cod/tod from the raw SLAM
  #                       survival files, so the healthcard figures reflect the
  #                       current run instead of a pinned one.
  #                       Verified: this reproduces the previously-frozen census
  #                       exactly on every column the analysis uses (cod/tod/le_wk);
  #                       the only cell that moves is 1 bw-class mouse (1/1315),
  #                       which is environment drift, not a wiring difference.
  #                       This analysis uses only new_class_bw; fat/gluc are never
  #                       touched.
  healthcard_path <- "../data/SLAM Healthcard reconciled.xlsx"
  census_path     <- "../../00a_clean_slam_c1-c10/data/census.csv"
  census_04_path  <- "../../04_create_census/output/slam_c1-c10_age_all_bwfatgluc/complete_census.csv"
  surv_c110_path  <- "../../00a_clean_slam_c1-c10/data/survival_2021-12-17.csv"   # tag-keyed,  tod = m/d/yy
  surv_c1618_path <- "../../00a_clean_slam_c16-c18/data/Survival_2025-02-14.csv"  # idno-keyed, tod = m/d/YYYY

  if (!file.exists(healthcard_path)) {
    stop("Missing the healthcard workbook:\n  ", normalizePath(dirname(healthcard_path), mustWork = FALSE),
         "/SLAM Healthcard reconciled.xlsx\n",
         "It is the one input to this analysis that the pipeline cannot regenerate -- ",
         "copy it into 95_healthcard_cod/data/ and re-run.")
  }
  ...
  # --- reconstruct class_census: 04 census + cod/tod from raw survival ---------
  class_census <- read.csv(census_04_path)                       # idno, new_class_bw, le_wk, ...
  # cod/tod live only in the raw survival CSVs -- 00a/00c/04 all trim them out on
  # the way through, so they are read straight from the raw files here.
  # c1-c10 is tag-keyed (join via the roster); c16-c18 is already idno-keyed.
  .ros  <- unique(data.frame(idno = census$idno, tag = trimws(as.character(census$tag)),
                             stringsAsFactors = FALSE))
  .s1   <- read.csv(surv_c110_path); .s1$tag <- trimws(as.character(.s1$tag))
  .s1   <- merge(.s1, .ros, by = "tag")
  ...
  .cod_tod <- rbind(.ct1, .ct2)
  .cod_tod <- .cod_tod[!duplicated(.cod_tod$idno), ]
...
```{r Incidence study}
  # Unified theme. Defined here (not in the later "compiled plots" chunk) because
  # this Incidence study chunk is the first to reference it -- defining it downstream
  # left it undefined at first use.
  ...
  #* Now, create a lifelong healthcard
    HC_relevant_LL <- classes_i %>%
      select(idno,le_wk,new_class_bw,created_date,recovery_date, tod,abscess:condition_to_death) 
...
```{r HC free study}
  #* Prepare cumulative incidence tibble
    ...
    #- Create a time-to-event tibble and join DOB
      HC_relevant_LL <- classes_i %>%
        ...
        mutate(
          dob = if_else(
            year(dob) < 100, # badly parsed DOBs have years like 15, 16, etc.
            update(dob, year = year(dob) + 2000), # convert 0015 → 2015
            dob
          )
        ) %>%
        mutate(
  ...
  #* Plot
    #- As raw days
      days_HCfree <- ggplot(cumulative_class_curves_day, aes(x = event_day, y = prop_disease_free_day, color = Class)) +
        geom_step(size = 1.1) +
...
```{r Cumulative HCs study}
  #* Compute cumulative HC tibbles
    ...
    #- Compute a version for only mice who had HCs
      cumulative_HC_day_events_only <- HC_relevant_LL %>%
        mutate(event_wk = event_day / 7) %>%
        filter(!is.na(event_wk)) %>% # excludes mice with 0 events
        mutate(Class = factor(new_class_bw, levels = c(1, 2, 3))) %>%
        group_by(Class) %>%
        mutate(class_size = n_distinct(idno)) %>%
        group_by(Class, event_wk) %>%
        summarise(
          total_HC = n(),
          class_size = first(class_size),
          .groups = "drop"
        ) %>%
        arrange(Class, event_wk) %>%
        group_by(Class) %>%
        mutate(
  ...
  #- Including only mice with HCs
    cum_inc_HCmice <- ggplot(cumulative_HC_day_events_only, aes(x = event_wk, y = cum_HC_per_mouse_events_only, color = Class)) +
      geom_step(size = 1.1) +
      scale_color_manual(values = class_colors) +
      labs(
  ...
  #- Including mice who never had HC
    cum_inc_allmice <- ggplot(cumulative_HC_day_all_mice, aes(x = event_wk, y = cum_HC_per_mouse_all, color = Class)) +
      geom_step(size = 1.1) +
      scale_color_manual(values = class_colors) +
      labs(
...
## Compile plots
```{r compiled plots}
  # unified_plot_theme is defined up in the "Incidence study" chunk (its first use).
  ...
  dev.off()
```  
## Pare and format plots to those actually included in supplemental and make specific changes
```{r finalized plots}
  # Function to rewrite class names
    relabel_classes <- function(df, var) {
      df %>%
        mutate({{ var }} := recode_factor(
          {{ var }},
          "1" = "Early-Peak-BW",
          "2" = "Stable BW",
          "3" = "Mid-Peak-BW"
  ...
  # Apply to relevant datasets
    bin_props_stacked_final <- relabel_classes(bin_props_stacked, new_class_bw)
    bin_props_yr_final <- relabel_classes(bin_props_yr, new_class_bw)
    cumulative_HC_day_events_only_final <- relabel_classes(cumulative_HC_day_events_only, Class)
    cumulative_HC_day_all_mice_final <- relabel_classes(cumulative_HC_day_all_mice, Class)
  # Set global plot theme
    final_plot_theme <- theme_minimal(base_size = 18) +
      theme(
        ...
        strip.text    = element_text(size = 14, face = "plain") # if using facets
      )
  # Finalize version of S8A (proportion of LL HCs stacked)
    S8A <- ggplot(bin_props_stacked_final, aes(x = factor(new_class_bw), y = proportion, fill = condition_bin)) +
      geom_bar(stat = "identity", position = "stack") +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
      scale_fill_manual(values = bin_colors, name = "# of HCs") +
      labs(
        title = "Condition Burden",
        x = "Class",
        y = "Proportion of Mice"
      ) + final_plot_theme

  # Finalize version of S8B (proportion of LL HCs stacked, year normalized)
    S8B <- ggplot(bin_props_yr_final, aes(x = factor(new_class_bw), y = proportion, fill = hc_yr_bin)) +
      geom_bar(stat = "identity", position = "stack") +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
      scale_fill_manual(values = bin_colors_per_yr, name = "HCs/Yr") +
      labs(
        title = "Condition Incidence / year",
        x = "Class",
        y = "Proportion of Mice"
      ) + final_plot_theme

  # Use the scale for S8C for next two plots
    max_y_S8C <- max(cumulative_HC_day_events_only_final$cum_HC_per_mouse_events_only, na.rm = TRUE)
  # Finalize version of S8C (cumulative incidence of HCs, only mice with events)
    S8C <- ggplot(cumulative_HC_day_events_only_final, aes(x = event_wk, y = cum_HC_per_mouse_events_only, color = Class)) +
      geom_step(size = 2.5) +
      scale_color_manual(values = class_colors_final) +
      scale_y_continuous(limits = c(0, max_y_S8C)) +
      labs(
        title = "Cumulative HCs per Mouse (Only Mice With Events)",
        x = "Weeks Since Birth",
        y = "Cumulative HCs / Mouse",
  ...
  # Finalize version of S8D (cumulative incidence of HCs, all mice)
    S8D <- ggplot(cumulative_HC_day_all_mice_final, aes(x = event_wk, y = cum_HC_per_mouse_all, color = Class)) +
      geom_step(size = 2.5) +
      scale_color_manual(values = class_colors_final) +
      scale_y_continuous(limits = c(0, max_y_S8C)) +
      labs(
  ...
  # Finalize version of S8E (cumulative incidence of HCs, all mice)
    condition_matrix_norm_renamed <- condition_matrix_norm
    colnames(condition_matrix_norm_renamed) <- recode(
      colnames(condition_matrix_norm_renamed),
      "Class 1" = "Early-Peak-BW",
      "Class 2" = "Stable BW",
      "Class 3" = "Mid-Peak-BW"
    )
    S8E <- pheatmap::pheatmap(
      condition_matrix_norm_renamed,
      scale = "none", # Fix inconsistent coloring
      cluster_rows = TRUE,
      cluster_cols = TRUE,
      annotation_row = sig_annot_timed,
  ...
  # Export all plots as high-quality pngs
    # Output folder, relative to this .rmd own dir (95_healthcard_cod/R/),
    # matching the input paths above (rendered from the .rmd directory).
    output_dir <- "../output"
    dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
    # Define export function
    export_plot <- function(plot_obj, filename, width = 8.5, height = 11, dpi = 1200) {
      ggsave(
    ...
    export_plot(S8A, "S8A.png")
    export_plot(S8B, "S8B.png")
    export_plot(S8C, "S8C.png")
    export_plot(S8D, "S8D.png")
    # Export S8E (pheatmap object)
    png(file.path(output_dir, "S8E.png"), width = 8.5, height = 11, units = "in", res = 1200, bg = "white")
    grid::grid.newpage()
    grid::grid.draw(S8E$gtable)
    dev.off()
```  

## Export cleaned data for Michel
```{r export data}
  # Clean dates on a copy of healthcard_final without touching the original
  healthcard_final_export <- healthcard_final %>%
    mutate(
      created_date  = as.Date(as.numeric(created_date), origin = "1899-12-30"),
      recovery_date = as.Date(recovery_date),
      dod           = as.Date(dod)
    )

  # Export healthcard_final
  write.csv(healthcard_final_export, file.path(output_dir, "healthcard_final.csv"), row.names = FALSE)

  # Export Fig S8A: parts-of-whole condition burden per mouse per class
  writexl::write_xlsx(bin_props_stacked_final, file.path(output_dir, "S8A.xlsx"))

  # Export Fig S8B: time-normalized incidence (HCs per year) per class
  writexl::write_xlsx(bin_props_yr_final, file.path(output_dir, "S8B.xlsx"))

  # Export hc_lifespan_data: per-mouse HC counts and binning (source data for S8A/S8B)
  writexl::write_xlsx(
    mutate(hc_lifespan_data, condition_bin = as.character(condition_bin), hc_yr_bin = as.character(hc_yr_bin)),
    file.path(output_dir, "hc_lifespan_data.xlsx")
  )
```
