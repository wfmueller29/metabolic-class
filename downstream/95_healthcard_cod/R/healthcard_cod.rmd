...
# Load in input data
```{r read in censuses}
  # Paths are relative to this .rmd's own dir (downstream/95_healthcard_cod/R/),
  # which is rmarkdown's default knit dir. `..`=95_healthcard_cod, `../../..`=repo root.
  #
  # INPUTS (see diagnose.R at repo root for the full provenance proof):
  #   - healthcard xlsx : the ONLY input unique to this analysis (hydrated raw layer)
  #   - roster census   : the pipeline's SLAM raw census (tag<->idno; verified identical)
  #   - class_census    : NOT frozen -- reconstructed here = 04 census (classes/le_wk)
  #                       joined with cod/tod from the raw SLAM survival files.
  #                       diagnose.R verified this reproduces the old frozen census
  #                       exactly except 1 bw-class mouse (env drift, 1/1315).
  healthcard_path <- "../data/SLAM Healthcard reconciled.xlsx"
  census_path     <- "../../../pipeline/00a_clean_slam_c1-c10/data/census.csv"
  census_04_path  <- "../../../pipeline/04_create_census/output/slam_c1-c10_age_all_bwfatgluc/complete_census.csv"
  surv_c110_path  <- "../../../pipeline/00a_clean_slam_c1-c10/data/survival_2021-12-17.csv"   # tag-keyed, tod = m/d/yy
  surv_c1618_path <- "../../../pipeline/00a_clean_slam_c16-c18/data/Survival_2025-02-14.csv"  # idno-keyed, tod = m/d/YYYY
  ...
  # cod/tod live only in the raw survival CSVs (the model drops them). Rebuild the
  # exact join diagnose.R proved airtight: c1-c10 via roster tag->idno, c16-c18 by idno.
...
```{r finalized plots}
  ...
  # Export all plots as high-quality pngs
    # Output folder, relative to this .rmd's own dir (downstream/95_healthcard_cod/R/),
    # matching the input paths above (rendered from the .rmd's directory).
    output_dir <- "../Outputs"
