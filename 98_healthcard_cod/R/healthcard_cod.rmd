...
```{r HC free study}
  ...
  #* Plot
    #- As raw days
      days_HCfree <- ggplot(cumulative_class_curves_day, aes(x = event_day, y = prop_disease_free, color = Class)) +
        geom_step(size = 1.1) +
...
```{r finalized plots}
  ...
  # Finalize version of S6A (proportion of LL HCs stacked)
    S6A <- ggplot(bin_props_stacked_final, aes(x = factor(new_class_bw), y = proportion, fill = condition_bin)) +
      geom_bar(stat = "identity", position = "stack") +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
      scale_fill_manual(values = bin_colors, name = "# of HCs") +
      labs(
  ...
  # Finalize version of S6B (proportion of LL HCs stacked, year normalized)
    S6B <- ggplot(bin_props_yr_final, aes(x = factor(new_class_bw), y = proportion, fill = hc_yr_bin)) +
      geom_bar(stat = "identity", position = "stack") +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
      scale_fill_manual(values = bin_colors_per_yr, name = "HCs/Yr") +
      labs(
  ...
  # Use the scale for S6C for next two plots
    max_y_S6C <- max(cumulative_HC_day_events_only_final$cum_HC_per_mouse_events_only, na.rm = TRUE)
  # Finalize version of S6C (cumulative incidence of HCs, only mice with events)
    S6C <- ggplot(cumulative_HC_day_events_only_final, aes(x = event_wk, y = cum_HC_per_mouse_events_only, color = Class)) +
      geom_step(size = 1.1) +
      scale_color_manual(values = class_colors_final) +
      scale_y_continuous(limits = c(0, max_y_S6C)) +
      labs(
        title = "Cumulative HCs per Mouse (Only Mice With Events)",
        x = "Weeks Since Birth",
        y = "Cumulative HCs / Mouse",
  ...
  # Finalize version of S6D (cumulative incidence of HCs, all mice)
    S6D <- ggplot(cumulative_HC_day_all_mice_final, aes(x = event_wk, y = cum_HC_per_mouse_all, color = Class)) +
      geom_step(size = 1.1) +
      scale_color_manual(values = class_colors_final) +
      scale_y_continuous(limits = c(0, max_y_S6C)) +
      labs(
  ...
  # Finalize version of S6E (cumulative incidence of HCs, all mice)
  ...
  # Export all plots as high-quality pngs
    ...
    export_plot(S6A, "S6A.png")
    export_plot(S6B, "S6B.png")
    export_plot(S6C, "S6C.png")
    export_plot(S6D, "S6D.png")
    # Export S6E (pheatmap object)
    png("Healthcard_PNGs/S6E.png", width = 8.5, height = 11, units = "in", res = 1200, bg = "white")
    grid::grid.newpage()
    grid::grid.draw(S6E$gtable)
    dev.off()
```  
