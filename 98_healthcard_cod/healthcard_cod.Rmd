...
```{r read in censuses}
class_census_path <- "../04_create_census/output/20250426_all_cohort/complete_census.csv"
census_path <- "data/census.csv"
healthcard_path <- "data/SLAM Healthcard.xlsx"

class_census <- read.csv(class_census_path)
census <- read.csv(census_path)
healthcard <- readxl::read_xlsx(healthcard_path)
healthcard <- as.data.frame(healthcard)
...
new_names <- c(
  ...
  recovery_date = "Recovery Date", condition = "Condition"
)
...
healthcard <- merge(census, healthcard, by = "tag", all.y = TRUE)
peek(healthcard)

healthcard$tag %in% str_splcensus$tag_history
apply(healthcard[, 20:68], 2, sum)
healthcard_sum <- aggregate(healthcard[, 20:68], by = list(healthcard$tag), sum)
```
